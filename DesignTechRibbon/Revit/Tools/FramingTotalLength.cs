...
namespace TotalLength
{
    ...
    public class FramingTotalLength : IExternalCommand
    {
        public class FramingFilter : ISelectionFilter
        {
            ...
            ElementSet elements)
        {
            ...
            try
            {
                ...
                FramingFilter selFilter = new FramingFilter();
                pickedRef = sel.PickElementsByRectangle(selFilter, "Select beams");

                // Measure their total length
                List<double> lengthList = new List<double>();
                foreach (Element e in pickedRef)
                {
                    double len = e.get_Parameter(BuiltInParameter.CURVE_ELEM_LENGTH).AsDouble();
                    lengthList.Add(len);
                }
                ...
                TaskDialog.Show("Beam Length", sb.ToString());
